...
const TemplateForm = () => {
  ...
  const formSections = [
    ...
    { id: 6, name: 'Education', component: 'education', required: true },
    { id: 7, name: 'Review & Generate', component: 'review', required: false }
  ];
  ...
  useEffect(() => {
    const loadResumeData = async () => {
      if (resumeId && user?.email) {
        ...
        setLoading(true);
        try {
          const data = await EncryptedFirebaseService.getResumeData(user.email, resumeId);
          setResumeInfo(data);
  ...
  const currentSection = formSections.find(section => section.id === activeSection);
  ...
  const generateResumeWithAI = async () => {
    ...
    try {
      ...
      if (!resumeInfo.personalDetail?.firstName || !resumeInfo.personalDetail?.email) {
  ...
  const renderFormSection = () => {
    ...
    switch (currentSection.component) {
      ...
      case 'review':
        return (
          <div className="p-5 shadow-lg rounded-lg border-t-4 mt-10 bg-white" style={{borderTopColor: 'rgb(246,196,158)'}}>
            <h2 className="font-bold text-xl text-slate-900">Review & Generate Resume</h2>
            <p className="text-sm text-slate-600 mb-4">Review your information and generate your resume</p>
            
            <div className="space-y-4 mb-6">
              <div>
                <label className="text-sm font-medium text-slate-900">Resume Title</label>
                <Input
                  value={resumeTitle}
              ...
              <div className="p-4 rounded-lg border" style={{
                background: 'linear-gradient(to right, rgba(246,196,158,0.1), rgba(255,228,196,0.1))',
                borderColor: 'rgba(246,196,158,0.3)'
              }}>
                <h3 className="font-medium mb-2" style={{color: 'rgb(139,69,19)'}}>Selected Template: {template?.name}</h3>
                <p className="text-sm" style={{color: 'rgb(160,82,45)'}}>{template?.description}</p>
              </div>
            </div>
  ...
  if (!template) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="animate-spin h-8 w-8 border-2 border-t-transparent rounded-full" style={{borderColor: 'rgb(246,196,158)', borderTopColor: 'transparent'}}></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 p-4 sm:p-6 lg:p-10 gap-6 lg:gap-10">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
            <div className="gap-3 hidden sm:flex">
              <Link to="/create/templates">
                <Button variant="outline" size="sm" className="bg-white border-slate-300 hover:border-[rgb(63,39,34)] text-slate-700 hover:bg-slate-50">
              ...
              {activeSection < formSections.length && (
                <Button
                  ...
                  style={{ 
                    background: 'linear-gradient(to right, rgb(246,196,158), rgb(236,186,148))',
                    '&:hover': {
                      background: 'linear-gradient(to right, rgb(236,186,148), rgb(226,176,138))'
                    }
                  }}
                  size="sm"
                  onMouseEnter={(e) => {
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-lg p-4 sm:p-6 border border-slate-200">
          <h2 className="font-bold text-xl mb-4 text-slate-900">Template Preview</h2>
          <div className="h-64 sm:h-80 lg:h-96 rounded-lg flex items-center justify-center border border-slate-200" style={{
            background: 'linear-gradient(to bottom right, rgba(246,196,158,0.1), rgba(255,228,196,0.1))'
          }}>
            <div className="text-center">
              <div className="text-4xl mb-3">📄</div>
              <h3 className="font-semibold text-lg text-slate-900">{template.name}</h3>
              <p className="text-sm text-slate-600 mt-2 px-4">{template.description}</p>
