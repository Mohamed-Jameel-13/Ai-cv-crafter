...
const azureApiVersion =
  import.meta.env.VITE_AZURE_OPENAI_API_VERSION || "2024-02-15-preview";
const azureDeploymentName =
  import.meta.env.VITE_AZURE_OPENAI_DEPLOYMENT_NAME || "gpt-4o";
...
Logger.log("Azure OpenAI Environment check:", {
  ...
  endpointFormat: azureEndpoint
    ? azureEndpoint.includes(".openai.azure.com")
      ? "Correct"
      : "Check format - should end with .openai.azure.com"
    : "Missing",
});
...
if (!azureEndpoint) {
  throw new Error(
    "Azure OpenAI endpoint is missing. Please add VITE_AZURE_OPENAI_ENDPOINT to your environment variables",
  );
}

if (!azureApiKey) {
  throw new Error(
    "Azure OpenAI API key is missing. Please add VITE_AZURE_OPENAI_API_KEY to your environment variables",
  );
}
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new Error(
            "Invalid Azure OpenAI API key. Please check your VITE_AZURE_OPENAI_API_KEY environment variable",
          );
        }

        if (response.status === 404) {
          throw new Error(
            "Azure OpenAI endpoint or deployment not found. Please check your VITE_AZURE_OPENAI_ENDPOINT and VITE_AZURE_OPENAI_DEPLOYMENT_NAME",
          );
        }
        ...
        throw new Error(
          `Azure OpenAI API error: ${response.status} ${response.statusText} - ${errorData.error?.message || "Unknown error"}`,
        );
      }
      ...
      if (data.choices && data.choices[0] && data.choices[0].message) {
        ...
        Logger.log(
          "✅ Successfully received response from Azure OpenAI GPT-4o",
        );
        return {
          response: {
            text: () => text,
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        throw new Error(
          "Network error: Unable to connect to Azure OpenAI. Please check your internet connection and endpoint URL",
        );
      }
