...
async function testAzureOpenAI() {
  ...
  const azureApiVersion = envVars.VITE_AZURE_OPENAI_API_VERSION || '2024-02-15-preview';
  const azureDeploymentName = envVars.VITE_AZURE_OPENAI_DEPLOYMENT_NAME || 'gpt-4o';
  ...
  if (azureEndpoint) {
    const isValidFormat = azureEndpoint.includes('.openai.azure.com') || azureEndpoint.includes('.cognitiveservices.azure.com');
    console.log(`✓ Endpoint Format: ${isValidFormat ? '✅ Correct' : '❌ Should end with .openai.azure.com or .cognitiveservices.azure.com'}`);
    console.log(`✓ Endpoint Protocol: ${azureEndpoint.startsWith('https://') ? '✅ HTTPS' : '❌ Should start with https://'}`);
    console.log(`✓ URL Type: ${azureEndpoint.includes('/chat/completions') ? '🔗 Full URL' : '🏠 Base URL'}`);
  }
  ...
  const isValidEndpoint = azureEndpoint.includes('.openai.azure.com') || azureEndpoint.includes('.cognitiveservices.azure.com');
  if (!isValidEndpoint) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        ...
        console.log('   - Verify the deployment name exists in Azure OpenAI Studio');
  ...
  console.log('• Documentation: https://docs.microsoft.com/en-us/azure/cognitive-services/openai/');
  console.log('');
  console.log('💡 Need help? Check DEPLOYMENT_GUIDE.md for detailed setup instructions.');
}
