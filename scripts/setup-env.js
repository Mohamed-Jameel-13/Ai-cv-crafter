...
async function setupEnvironment() {
  ...
  console.log('This script will help you set up your environment variables for Azure OpenAI.\n');
  ...
  const azureEndpoint = await askQuestion('Enter your Azure OpenAI endpoint URL: ');
  const azureApiKey = await askQuestion('Enter your Azure OpenAI API key: ');
  const azureApiVersion = await askQuestion('Enter API version (press Enter for default 2024-02-15-preview): ') || '2024-02-15-preview';
  const azureDeploymentName = await askQuestion('Enter deployment name (press Enter for default "gpt-4o"): ') || 'gpt-4o';
  ...
  const firebaseStorageBucket = await askQuestion('Enter Firebase Storage Bucket: ');
  const firebaseMessagingSenderId = await askQuestion('Enter Firebase Messaging Sender ID: ');
  const firebaseAppId = await askQuestion('Enter Firebase App ID: ');
  
  console.log('\n👤 Authentication Configuration:');
  const clerkKey = await askQuestion('Enter Clerk Publishable Key (optional): ');
